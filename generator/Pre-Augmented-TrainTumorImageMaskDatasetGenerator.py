...
#
# 2025/01/01 Pre-Aumgented-TrainTumorImageMaskDatasetGenerator.py
  ...
  def generate_files(self, image_file, index, output_dir, mask=False):
     ...
     if mask:
       if np.all(image==0):
         ...
         return False
       # 2024/12/17 Added the following lines
       if np.all(image==255):
         print("---Skip all white mask {}".format(image_file))
         return False
...
if __name__ == "__main__":
  try:
    output_dir  = "./Prostate-Train-Tumor-augmented-master/"
